...
exports.userRegistration = async (req, res) => {
  ...
  try {
    ...
    if (foundUser) {
      res.status(400).send({ message: "the E-mail was already registered" });
      return 
    }
...
exports.userlogin=(req,res)=>{
  ...
  User.findOne({email:email}).then(userData=>{
    ...
    bcrypt.compare(password, userData.password).then(match=>{
      ...
      return res.status(401).json({message:'Your account has Banned by the admin!'})
      }
...
exports.token_send=async (req,res)=>{
  ...
  try {
    ...
    res.render('forgetPassword',{ message: `Password reset email sented at ${email}` });
    return 
